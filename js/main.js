...
for (let i = 0; i < arr.length; i++) {
  if (arr[i].match(/^[А-Яа-я]+\s?[А-Яа-я]+?[(а-я),\s]+?\.\s/)) {
    filteredArr.push(arr[i]);
  }
}

for (let i = 0; i < filteredArr.length; i++) {
  filteredArr[i] = `${i + 1}) ${filteredArr[i]}`;
}
...
for (let i = 0; i < roles.length; i++) {
  ...
  h3.innerText = roles[i];
  group.append(h3);

  for (let j = 0; j < filteredArr.length; j++) {
    let re = new RegExp('^[0-9]+\\)\\s' + roles[i] + '([(а-я),\\s]+)?\\.\\s');

    if (filteredArr[j].match(re)) {
      let reSecond = new RegExp(roles[i] + '\\.?\\s?');
      let filteredString = filteredArr[j].replace(reSecond, '');

      let p = document.createElement('p');
      p.innerText = filteredString;
      group.append(p);
    }
  }
